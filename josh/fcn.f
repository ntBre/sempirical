!
! This is a user-supplied subroutine, which has to return FVEC for a given set of X
! 
...
C
      ...
      CALL FLUSH(6)
! 
!  Write the parameters to the PARAMS.DAT file, which is read by MSINDO
...
!
!  Execute MOPAC and the program that analyzes the output files
!
...
!
!  Read in the semiempirical energies
!
       RMSD=0.0
c Reads the energies of the different structures
         jj=0
      do i=1, nmolec
        do j=1, nstruct(i)
           jj=jj+1
...
!
     ...
     .          eab(jj), e1(jj), eab(jj)-e1(jj)
         ...
         write(6,*) 
       enddo
c RMSD calculated accounts for the weighting of eRnI to eRnF points
      WRITE(6,'(A4,1x,F17.12)') 'RMSD',SQRT(RMSD/REAL(m-sum(ngeoms)))
      WRITE(6,*)
      CALL FLUSH(6)

c     Calculate the weights on FVEC before returning the differences
...
c
c Write all parameters used for each iteration to allparams.out
